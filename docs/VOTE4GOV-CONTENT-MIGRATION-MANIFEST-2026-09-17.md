...
Community / territorial target URLs now exist canonically at VoiceOpenGov:
...
These VoiceOpenGov pages intentionally do not invent members, groups, contacts, events or current topics. Until verified regional data exists they expose the state transparently as `Community im Aufbau` and route detailed evidence work to eDebatte.
...
- remove legacy territorial pages from the Vote4Gov sitemap when their migration is complete;
- replace self-canonicals on migrated community pages with 301 redirects;
...
- The remaining states receive a shared, multilingual, fail-honest regional page.
...
- State pages provide VoiceOpenGov participation and eDebatte evidence-work handoffs.
- Canonical, hreflang, OpenGraph and structured-data metadata are generated for the state pages.
- Sitemap coverage is generated from the same canonical state registry.
...
- Update Vote4Gov sitemap, canonicals, internal links and breadcrumbs accordingly.
