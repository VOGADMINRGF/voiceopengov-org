...
export type Row = { name: string; ok: boolean; ms: number; error?: string; skipped?: boolean };
...
async function timed(name: string, fn: () => Promise<MaybeRow>): Promise<Row> {
  ...
  try {
    ...
    return { name, ok: false, ms: Math.round(performance.now() - t0), error: e?.message ?? String(e) };
  }
}
...
export async function runSystemMatrix(): Promise<Row[]> {
  return Promise.all<Row>([
    ...
    timed("redis", async () => {
      ...
      return { name: "redis", ok: false, error: typeof r === "string" ? r : "unknown" };
    }),

    // Neo4j (Bolt)
    timed("neo4j:bolt", async () => ({ name: "neo4j:bolt", ok: await neo4jPing() })),
