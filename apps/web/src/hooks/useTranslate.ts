...
type BatchOut = Record<string, { text: string; result: string; cached: boolean }[]>;

export function useTranslate() {
  const [loading, setLoading] = useState(false);
  const [error, setErr] = useState<string | null>(null);

  const translate = useCallback(async (text: string, to: string | string[], from?: string) => {
    ...
    try {
      ...
      setErr(e.message || String(e)); throw e;
...
export function useBatchTranslate() {
  ...
  const translateMany = useCallback(async (texts: string[], to: string | string[], from?: string) => {
    setLoading(true); setErr(null);
    try {
      const r = await fetch("/api/translate", {
        method: "POST",
