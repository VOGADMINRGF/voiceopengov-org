...
function b64url(bytes: Uint8Array) {
  ...
  if (typeof g.Buffer !== "undefined" && typeof g.Buffer.from === "function") {
    ...
    // Edge Runtime: btoa ist verfügbar
    let bin = "";
...
export async function middleware(req: NextRequest) {
  ...
  if (req.method === "OPTIONS") return (NextResponse as any).next();
  ...
  const res = (NextResponse as any).next();
  ...
  if (
    ...
    if (!headerToken || !cookieToken || headerToken !== cookieToken) {
      return new Response("Forbidden (CSRF)", {
        status: 403,
        headers: { "X-Request-Id": rid }
      });
    }
  ...
  // sicherstellen, dass CSRF-Cookie existiert (nicht HttpOnly, damit Client es fürs Double-Submit lesen kann)
