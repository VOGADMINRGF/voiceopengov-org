...
export async function POST() {
  ...
  const ops = [
    db.collection("votes").deleteMany(byUser),
    db.collection("contributions").deleteMany(byUser),
    db.collection("userprofiles").deleteOne(byUser),
    db.collection("users").deleteOne(byId),
  ];
