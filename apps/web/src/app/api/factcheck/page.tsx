        ...
        if (st.ok && (sj.job.status === 'COMPLETED' || sj.job.status === 'FAILED')) {
          setResult({
            ...
            verdicts: sj.claims.map((c: any) => ({
              id: c.id, text: c.text,
              verdict: c.consensus?.verdict, confidence: c.consensus?.confidence
            }))
          });
          setLoading(false);
          return;
    ...
    <main className="max-w-3xl mx-auto p-6 space-y-4">
      ...
      {result && (
        <div className="border rounded p-4 space-y-2">
          <div className="text-sm text-gray-600">Job #{result.job.jobId} – {result.job.status}</div>
          {result.verdicts.map((v: any) => (
            <div key={v.id} className="p-3 bg-gray-50 rounded">
              <div className="font-medium">{v.text}</div>
              <div>Verdict: <b>{v.verdict ?? 'n/a'}</b> (confidence {v.confidence ?? 'n/a'})</div>
            </div>
          ))}
        </div>
      )}
