...
export async function GET(_: Request, { params }: { params: { contributionId: string } }) {
  ...
  if (!job) {
    return NextResponse.json({ ok: false, reason: "No job found for contributionId", results: [] }, { status: 404 });
  }
  ...
  return NextResponse.json({ ok: true, job: { id: job.id, status: job.status, tokensUsed: job.tokensUsed, durationMs: job.durationMs }, results });
}
