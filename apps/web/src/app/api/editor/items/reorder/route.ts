...
const ReorderSchema = z.object({
  orders: z.array(
    z.object({
      id: z.string().min(1, "orders[].id required"),
      order: z.number().int().nonnegative("orders[].order must be >= 0"),
    })
  ),
});
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    const missingIds = orders.map((o) => o.id).filter((id) => !foundIds.some((f) => f.id === id));
