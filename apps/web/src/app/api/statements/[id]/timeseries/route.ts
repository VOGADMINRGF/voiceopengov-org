...
export async function GET(_: Request, { params }: { params: { id: string }}) {
  ...
  const agg = await votes.aggregate([
    ...
    { $project: {
        day: { $toDate: { $subtract: ["$ts", { $mod: ["$ts", 24*3600*1000] }] } },
        value: 1
    }},
    { $group: { _id: { day: "$day", value: "$value" }, n: { $sum: 1 } }},
    { $group: {
        _id: "$_id.day",
        agree:    { $sum: { $cond: [{ $eq: ["$_id.value", "agree"]    }, "$n", 0] } },
        neutral:  { $sum: { $cond: [{ $eq: ["$_id.value", "neutral"]  }, "$n", 0] } },
        disagree: { $sum: { $cond: [{ $eq: ["$_id.value", "disagree"] }, "$n", 0] } },
    }},
    { $sort: { _id: 1 } }
  ]).toArray();
  ...
  const series = agg.map(a => ({
    ...
    agree: a.agree, neutral: a.neutral, disagree: a.disagree,
