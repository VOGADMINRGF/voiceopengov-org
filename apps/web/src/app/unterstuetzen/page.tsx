...
export default async function SupportPage({ searchParams }: { searchParams?: Promise<{ cancelled?: string }> }) {
  ...
  return <main className="min-h-screen bg-[#020617] px-4 py-16 text-[#f8fafc] sm:px-6" dir={locale === "ar" ? "rtl" : "ltr"}>
    <div className="mx-auto max-w-5xl"><p className="text-sm font-bold uppercase tracking-[.2em] text-cyan-300">{strings.eyebrow}</p><h1 className="mt-4 max-w-4xl text-4xl font-black tracking-tight sm:text-6xl">{strings.title}</h1><p className="mt-5 max-w-3xl text-lg leading-relaxed text-slate-300">{strings.body}</p>
      ...
      <section className="mt-8 grid gap-4 md:grid-cols-3" aria-label="VoiceOpenGov pricing">
        {[{ title: strings.membershipTitle, description: strings.membershipDescription }, { title: strings.supportingTitle, description: strings.supportingDescription }, { title: strings.fundingTitle, description: strings.fundingDescription }].map((tier) => <article key={tier.title} className="rounded-2xl border border-slate-800 bg-slate-900/65 p-5"><h2 className="text-lg font-extrabold text-cyan-100">{tier.title}</h2><p className="mt-2 text-sm leading-relaxed text-slate-300">{tier.description}</p></article>)}
      </section>
