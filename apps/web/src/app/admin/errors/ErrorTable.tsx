...
export default function ErrorTable({ initial }: { initial: Row[] }) {
  ...
  useEffect(() => {
    ...
    debounceRef.current = window.setTimeout(() => fetchPage(1), 250) as unknown as number;
    return () => {
      if (debounceRef.current) window.clearTimeout(debounceRef.current);
    };
  }, [q, status, fetchPage]);
  ...
  const toggleResolved = async (id: string) => {
    ...
    const filterWouldHide =
      (status === "open" && nextResolved) || (status === "resolved" && !nextResolved);
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2 items-center">
        ...
        <button className="border rounded px-3 py-1" onClick={exportCSV} disabled={!rows.length}>
          CSV Export
        </button>
              ...
              <td>
                <span
                  className={`px-2 py-1 rounded ${
                    err.resolved ? "bg-green-200 text-green-900" : "bg-red-200 text-red-900"
                  }`}
                >
