...
const TOPIC_KEYWORDS: Record<string, RegExp[]> = {
  "Klimawandel": [/klima/i, /co2/i, /treibhaus/i, /wärm(e|er)|hitze/i],
  "Infrastruktur": [/öpnv/i, /bus|bahn/i, /straß|brück/i, /internet|breitband/i],
  "Teilhabe": [/inklusion|teilhabe/i, /barrierefrei/i, /beteiligung/i],
  "Sicherheit": [/kriminal|polizei|beleuchtung|dunkel/i],
};

function scoreTopics(text: string): TopicScore[] {
  const scores: TopicScore[] = [];
  for (const [name, regs] of Object.entries(TOPIC_KEYWORDS)) {
    let hits = 0; for (const r of regs) if (r.test(text)) hits++;
    if (hits > 0) scores.push({ name, confidence: Math.min(1, 0.4 + hits * 0.2) });
  }
...
export async function analyzeContributionHeuristic(req: ContributionAnalysisRequest) {
  ...
  if (statements[0].type === "ja/nein") suggestions.push("Du könntest daraus eine Swipe-Frage machen.");
  if (topics.some(t=>/Infrastruktur/i.test(t.name))) suggestions.push("Das Thema könnte mit bestehenden Berichten zu Mobilität verknüpft werden.");
