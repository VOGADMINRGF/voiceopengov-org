...
export default function PolitikDashboard() {
  ...
  if (!hasAccess) return <div className="text-center text-gray-400 mt-16">Zugriff verweigert</div>;

  return (
    <main className="max-w-6xl mx-auto px-4 py-12 space-y-8">
      <h1 className="text-3xl font-bold text-blue-700 mb-2">Politik-Dashboard</h1>
        ...
        children={[
          ...
          <div key="streams">
            ...
            {showStreamForm && (
              <StreamForm
                stream={editStream}
                onSave={() => { setShowStreamForm(false); setEditStream(null); }}
                onCancel={() => { setShowStreamForm(false); setEditStream(null); }}
                user={user}
              />
            )}
          </div>,
          ...
          <div key="statements">
            ...
            {showStatementForm && (
              <StatementForm
                ...
                onSubmit={() => { setShowStatementForm(false); setEditStatement(null); }}
                onCancel={() => { setShowStatementForm(false); setEditStatement(null); }}
                user={user}
              />
            )}
          </div>,
          ...
          <div key="reporting" className="mt-8">
            {user?.premium ? (
              <>
                <h2 className="text-2xl font-bold mb-4">Monatsreporting für deine Region/Rolle</h2>
              ...
              <div className="text-gray-400 text-sm text-center mt-10">
                ...
                <p>
                  Das Monatsreporting steht nur Premium-Accounts zur Verfügung.<br />
                  <a href="/upgrade" className="text-indigo-600 hover:underline">Jetzt upgraden</a>
                </p>
              </div>
            )}
          </div>,
