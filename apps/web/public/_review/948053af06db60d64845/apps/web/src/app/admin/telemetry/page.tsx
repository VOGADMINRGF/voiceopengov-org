...
async function getAI() {
  const r = await fetch(absUrl("/api/admin/telemetry/ai"), { cache: "no-store" });
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {data.providers.map((p: any) => (
            <div key={p.name} className={`border rounded p-4 ${p.ok ? "border-green-400" : p.skipped ? "border-yellow-400" : "border-red-400"}`}>
