...
export default function MonitoringControls() {
  const [settings, setSettings] = useState<AlertSettings>({ enabled: true, recipients: [] });
  ...
  useEffect(() => {
    (async () => {
      const r = await fetch("/api/admin/alerts/settings", { cache: "no-store" });
  ...
  return (
    <div className="rounded-xl border p-4 space-y-4">
      ...
      <label className="block">
        <div className="text-sm text-gray-600 mb-1">Empfänger (Komma oder Zeilenumbruch)</div>
        <textarea
