  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-20 space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <h1 className="text-3xl font-bold text-coral">Deine Streams verwalten</h1>
      ...
      {filteredStreams.length === 0 ? (
        <p className="text-gray-500 text-center">Keine passenden Streams gefunden.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {filteredStreams.map((stream) => (
            <div key={stream.id} className="border border-gray-200 rounded-lg p-4 bg-white shadow-sm">
              <h2 className="text-xl font-semibold text-foreground mb-1">{stream.title}</h2>
              ...
              <div className="mt-3">
                <button
                  ...
                  onClick={() => alert("Vorgemerkt – Benachrichtigung wird vorbereitet.")}
                >
                  🔔 Vormerken
                </button>
              </div>
