...
type Topic = { id: string; slug: string; title: string; description?: string | null; locale: string };
      ...
      <div className="mx-auto max-w-3xl p-6">
        ...
        <div className="border rounded p-3 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            <input className="border rounded px-2 py-1" placeholder="slug"
              value={form.slug ?? ""} onChange={(e) => setForm({ ...form, slug: e.target.value })} />
            <input className="border rounded px-2 py-1" placeholder="title"
              value={form.title ?? ""} onChange={(e) => setForm({ ...form, title: e.target.value })} />
            <input className="border rounded px-2 py-1" placeholder="locale"
              value={form.locale ?? "de"} onChange={(e) => setForm({ ...form, locale: e.target.value })} />
            <input className="border rounded px-2 py-1 md:col-span-3" placeholder="description"
              value={form.description ?? ""} onChange={(e) => setForm({ ...form, description: e.target.value })} />
          </div>
          <div className="mt-2">
            <button onClick={create} className="border rounded px-3 py-2">Anlegen</button>
          </div>
        </div>
        ...
        <div className="space-y-2">
          {topics.map(t => (
            <div key={t.id} className="border rounded p-3">
              <div className="font-semibold">{t.title} <span className="opacity-60">({t.slug})</span></div>
