...
export function requireEditorAuth(req: Request): NextResponse | null {
  ...
  const token = (hdr?.startsWith("Bearer ") ? hdr.slice(7) : null) || req.headers.get("x-editor-token") || undefined;

  if (!process.env.EDITOR_TOKEN) {
    return NextResponse.json({ error: "EDITOR_TOKEN_NOT_CONFIGURED" }, { status: 500 });
  }
