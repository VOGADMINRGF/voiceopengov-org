...
export default function RegionConsent({ className = "" }: { className?: string }) {
  const { region, checking, askGeolocationOnce, setManualRegion } = useRegionPreference();
  const [open, setOpen] = useState(false);
  const [status, setStatus] = useState<PermissionState | "unknown">("unknown");

  useEffect(() => {
    if (!("permissions" in navigator)) { setStatus("unknown"); return; }
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black/40 ${className}`}>
      <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-xl p-6 w-[min(94vw,560px)]">
        <h2 className="text-xl font-semibold mb-2">Standort für lokale Inhalte verwenden?</h2>
        <p className="text-sm text-neutral-600 dark:text-neutral-300">
          Wir fragen einmalig nach deiner aktuellen Position, um <b>nahe Themen</b> zu priorisieren.
          Deine Position wird <b>nicht gespeichert</b>, außer du möchtest deine Region im Profil übernehmen.
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          >Region manuell wählen</a>
        </div>

        {status === "denied" && (
          <p className="mt-3 text-xs text-red-600">
            Zugriff aktuell blockiert. Bitte erlaube Standort in den Browser-Einstellungen.
          </p>
        )}
      </div>
    </div>
