...
try {
  ...
  if (await skip.count() !== 1) {
    ...
    const landed = await skipPage.evaluate(() => location.hash === "#main-content" || document.activeElement?.id === "main-content");
  ...
  const narrow = await browser.newContext({ viewport: { width: 320, height: 700 }, screen: { width: 320, height: 700 }, hasTouch: true, reducedMotion: "reduce" });
  for (const path of ["/", "/mitmachen"]) {
    await checkPage(narrow, path);
  }
  await narrow.close();

  const rtl = await browser.newContext({ viewport: { width: 390, height: 844 }, locale: "ar", hasTouch: true, reducedMotion: "reduce" });
...
console.log("Public browser accessibility regression passed: WCAG 2.x/2.2 AA automated checks, one-h1 structure, lang/dir, skip link, overflow, desktop, mobile and RTL.");
