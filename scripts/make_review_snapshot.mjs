#!/usr/bin/env node
// scripts/make-review-snapshot.mjs
...
async function main() {
  ...
  if (args[0] === "--clean" && args[1]) {
    ...
    if (await fs.pathExists(dir)) {
      await fs.remove(dir);
      console.log(`✓ removed snapshot ${args[1]} (${dir})`);
    } else {
  ...
  // Welche Dateien wollen wir wirklich zeigen? -> alles außer Excludes.
  ...
  // Sicherheitsgeländer: harte Dateinamen blocken (falls trotz Excludes auftauchen)
  const HARD_DENY = [/\.env(\.|$)/i, /^secrets\//i];
  const files = entries
    .map(toPosix)
    .filter((p) => !HARD_DENY.some((re) => re.test(p)));

  if (!files.length) {
    console.error("No files matched. Adjust DEFAULT_EXCLUDES if needed.");
    process.exit(1);
  }

  await fs.ensureDir(dest);

  // Kopieren & Metadaten sammeln
  const index = [];
  let copied = 0;
  for (const rel of files) {
    ...
    copied++;
    if (copied % 500 === 0) process.stdout.write(`...${copied}\r`);
  }

  // Index-Dateien schreiben
  await fs.writeJson(path.join(dest, "index.json"), { createdAt: Date.now(), root: "/", files: index }, { spaces: 2 });

  // Simple Explorer-Seite
...
small{color:#9ca3af}
.row{display:flex; gap:12px; align-items:center; padding:6px 0; border-bottom:1px solid #111827;}
.path{flex:1; overflow:auto; white-space:nowrap;}
.badge{font-size:12px; color:#d1d5db;}
</style>
</head><body>
<h1>Review Snapshot <small>${token}</small></h1>
<p>Nur wer den Link hat, sieht das. <a href="./index.json" target="_blank">index.json</a></p>
<input id="q" placeholder="Datei suchen (Pfad enthält) …"/>
<div id="list"></div>
<script type="module">
...
const res=await fetch('./index.json',{cache:'no-store'}); const data=await res.json();
let files=data.files;
function render(filter=""){
  list.innerHTML="";
  const f=filter.trim().toLowerCase();
  for(const it of files){
    if(f && !it.path.toLowerCase().includes(f)) continue;
...
render();
  ...
  console.log(`\n✓ Snapshot ready: ${dest}`);
  console.log(`   Preview path: /_review/${token}/index.html`);
  console.log(`   Example file: /_review/${token}/${files[0]}`);
  console.log(`   Files total : ${files.length}`);
  console.log(`\nRun:   pnpm --filter ./apps/web dev   # local check`);
  console.log(`Deploy a preview branch to Vercel and send me the URL.\n`);
}
